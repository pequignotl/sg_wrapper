    ...
    def get_new_shotgun_auth_info(self, scriptName=''):
        ...
        if not scriptEntity:
            ...
            if len(archivedScripts) > 0:
                ...
                # try/except to be compatible with old and new version of Shotgun
                # To remove after shotgun update
                try:
                    suid_generated = str(uuid.uuid4()).replace('-', '')
                    scriptEntity = self.create('ApiUser',
                                               firstname=scriptName,
                                               lastname='1.0',
                                               description='autogenerated key',
                                               permission_rule_set=adminPermission,
                                               salted_password=suid_generated,
                                               sg_public_password=suid_generated,
                                               )
                except:
                    scriptEntity = self.create('ApiUser', firstname=scriptName, lastname='1.0',
                                               description='autogenerated key', permission_rule_set=adminPermission)
                    scriptEntity.reload()
                    self._sg.update('ApiUser', scriptEntity['id'],
                                    {'sg_public_password': scriptEntity['salted_password']})
